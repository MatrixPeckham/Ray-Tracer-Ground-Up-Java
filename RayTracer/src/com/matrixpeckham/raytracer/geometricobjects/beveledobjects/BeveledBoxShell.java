...
public class BeveledBoxShell extends Compound {
    ...
    public BeveledBoxShell(Point3D minCorner,Point3D maxCorner,double bevelRadius){
        super();
        p0.setTo(minCorner);
        p1.setTo(maxCorner);
        rb=bevelRadius;
    ...
    private void buildCompound() {
        double startCyl = 0;
        double endCyl = 90;
        Instance top_front_edge = new Instance(new ConvexPartCylinder(-(p1.x - p0.x
	...
	// top back (-ve z)
        Instance top_back_edge = new Instance(new ConvexPartCylinder(-(p1.x - p0.x - 2
                * rb) / 2, (p1.x - p0.x - 2 * rb) / 2, rb,startCyl+90,endCyl+90));	// top back edge
	...
	// top right (+ve x)
        Instance top_right_edge = new Instance(new ConvexPartCylinder(-(p1.z - p0.z
                - 2 * rb) / 2, (p1.z - p0.z - 2 * rb) / 2, rb,startCyl+90,endCyl+90)); // top right edge
	...
	// top left (-ve x)
        Instance top_left_edge = new Instance(new ConvexPartCylinder(-(p1.z - p0.z - 2
                * rb) / 2, (p1.z - p0.z - 2 * rb) / 2, rb,startCyl+180,endCyl+180)); // top left edge
	...
	// bottom edges  (-ve y)
	// bottom front  (+ve z)
        Instance bottom_front_edge = new Instance(new ConvexPartCylinder(-(p1.x - p0.x
                - 2 * rb) / 2, (p1.x - p0.x - 2 * rb) / 2, rb,startCyl+270,endCyl+270));	// bottom fromt edge
	...
	// bottom back  (-ve z)
        Instance bottom_back_edge = new Instance(new ConvexPartCylinder(-(p1.x - p0.x
                - 2 * rb) / 2, (p1.x - p0.x - 2 * rb) / 2, rb,startCyl+180,endCyl+180));	// bottom back edge
	...
	// bottom right (-ve x, -ve y)
        Instance bottom_right_edge = new Instance(new ConvexPartCylinder(-(p1.z - p0.z
	...
	// bottom left (-ve x, -ve y)
        Instance bottom_left_edge = new Instance(new ConvexPartCylinder(-(p1.z - p0.z
                - 2 * rb) / 2, (p1.z - p0.z - 2 * rb) / 2, rb,startCyl+270,endCyl+270)); // bottom left edge
	...
	// vertical right front  (+ve x, +ve z)
        Instance vertical_right_front_edge = new Instance(new ConvexPartCylinder(p0.y
	...
	// vertical left front  (-ve x, +ve z)
        Instance vertical_left_front_edge = new Instance(new ConvexPartCylinder(p0.y
	...
	// vertical left back  (-ve x, -ve z)
        Instance vertical_left_back_edge = new Instance(new ConvexPartCylinder(p0.y
	...
	// vertical right back  (+ve x, -ve z)
        Instance vertical_right_back_edge = new Instance(new ConvexPartCylinder(p0.y
	...
	// corner spheres
	// top right front
        ConvexPartSphere top_right_front_corner = new ConvexPartSphere(new Point3D(p1.x - rb, p1.y
                - rb, p1.z - rb), rb,0,90,0,90);
        objects.add(top_right_front_corner);

	// top left front  (-ve x)
        ConvexPartSphere top_left_front_corner = new ConvexPartSphere(new Point3D(p0.x + rb, p1.y
                - rb, p1.z - rb), rb,270,360,0,90);
        objects.add(top_left_front_corner);

	// top left back
        ConvexPartSphere top_left_back_corner = new ConvexPartSphere(new Point3D(p0.x + rb, p1.y
                - rb, p0.z + rb), rb,180,270,0,90);
        objects.add(top_left_back_corner);

	// top right back
        ConvexPartSphere top_right_back_corner = new ConvexPartSphere(new Point3D(p1.x - rb, p1.y
                - rb, p0.z + rb), rb,90,180,0,90);
        objects.add(top_right_back_corner);

	// bottom right front
        ConvexPartSphere bottom_right_front_corner = new ConvexPartSphere(new Point3D(p1.x - rb,
                p0.y + rb, p1.z - rb), rb,0,90,90,180);
        objects.add(bottom_right_front_corner);

	// bottom left front
        ConvexPartSphere bottom_left_front_corner = new ConvexPartSphere(new Point3D(p0.x + rb, p0.y
                + rb, p1.z - rb), rb,270,360,90,180);
        objects.add(bottom_left_front_corner);

	// bottom left back
        ConvexPartSphere bottom_left_back_corner = new ConvexPartSphere(new Point3D(p0.x + rb, p0.y
                + rb, p0.z + rb), rb,180,270,90,180);
        objects.add(bottom_left_back_corner);

	// bottom right back
        ConvexPartSphere bottom_right_back_corner = new ConvexPartSphere(new Point3D(p1.x - rb, p0.y
