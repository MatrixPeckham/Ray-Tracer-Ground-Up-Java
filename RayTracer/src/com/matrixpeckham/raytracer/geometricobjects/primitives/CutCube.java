...
public class CutCube extends GeometricObject {
    ...
    public CutCube(double x0, double x1, double y0, double y1, double z0, double z1, double rad) {
        this.x0 = x0;
        this.x1 = x1;
        this.y0 = y0;
        this.y1 = y1;
	...
	// find smallest exiting t value
        ...
        if (t0 < t1 && t1 > Utility.EPSILON) {  // condition for a hit
            if (t0 > Utility.EPSILON) {
                if(s.distSquared(ray.o.add(Vector3D.mul(t0, ray.d)))<sphereRad*sphereRad){
                    return false;
                }
                sr.lastT = t0;  			// ray hits outside surface
                sr.normal .setTo( getNormal(face_in));
            } else {
                if(s.distSquared(ray.o.add(Vector3D.mul(t1, ray.d)))<sphereRad*sphereRad){
                    return false;
                }
