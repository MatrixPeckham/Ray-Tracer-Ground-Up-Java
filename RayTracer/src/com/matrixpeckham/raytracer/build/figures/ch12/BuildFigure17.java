...
//	stereoPtr.useTransverseViewing();
	...
	for (int r = 0; r < numRows; r++)  			// xw direction
		for (int c = 0; c < numColumns; c++) {		// zw direction
			...
			if (abs(rowTest) >= (numParkRows / 2) || abs(columnTest) >= (numParkColumns / 2)) {
				...
				mattePtr.setKa(0.4); 
				mattePtr.setKd(0.6);			
				mattePtr.setCd(	minColor + Utility.randDouble() * (maxColor - minColor), 
				...
				Box buildingPtr = new  Box(new Point3D(xmin, ymin, zmin), new Point3D(xmax, ymax, zmax));
	...
	Box parkPtr = new Box( 	new Point3D(-a * numParkRows / 2, 0.0, -b * numParkColumns / 2), 
