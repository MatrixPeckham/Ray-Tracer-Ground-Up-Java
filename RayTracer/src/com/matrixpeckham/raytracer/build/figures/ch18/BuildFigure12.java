...
import com.matrixpeckham.raytracer.lights.FakeSphericalLight;
...
int numSamples = 100;   
	...
	emissivePtr.scaleRadiance(40.0);   // can be different from the lightPtr
	...
	spherePtr.setSampler(samplerPtr);
	spherePtr.setShadows(false);
	w.addObject(spherePtr);
	
		
	FakeSphericalLight lightPtr = new FakeSphericalLight();
	lightPtr.setLocation(center);
	lightPtr.scaleRadiance(250.0);       // for Figure 18.12(a)
//	lightPtr.scaleRadiance(3.0);			// for Figure 18.12(b)
	lightPtr.setJitterAmount(radius);
	lightPtr.setExp(2);//comment out for b
	...
	Box boxPtr0 = new Box(new Point3D(- 1.5 * gap - 2.0 * boxWidth, 0.0, -0.5 * boxDepth), 
							new Point3D(-1.5 * gap  - boxWidth, boxHeight, 0.5 * boxDepth)); 
	boxPtr0.setMaterial(mattePtr1);
	w.addObject(boxPtr0);
	
	Box boxPtr1 = new Box(new Point3D(- 0.5 * gap - boxWidth, 0.0, -0.5 * boxDepth), 
	...
	Box boxPtr3 = new Box(new Point3D(1.5 * gap + boxWidth, 0.0, -0.5 * boxDepth), 
