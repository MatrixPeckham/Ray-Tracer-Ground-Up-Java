...
//	pinholePtr.setViewDistance(600.0);		// for 475 x 250
	...
	Plane frontWallPtr = new Plane(new Point3D(0, 0, 51), new Normal(0, 0, -1));
	...
	Plane leftWallPtr = new Plane(new Point3D(-15, 0, 0), new Normal(1, 0, 0));
	...
	Plane rightWallPtr = new Plane(new Point3D(15, 0, 0), new Normal(-1, 0, 0));
	...
	for (int iz = 0; iz < numZColumns; iz++) {     // to the front
		...
		for (int ix = 0; ix < numXPlanks; ix++) {  // to the right
			...
			woodPtr.translate(10.0 * (2.0 * Utility.randDouble() - 1.0), 20.0 * (2.0 * Utility.randDouble() - 1.0), 30.0 * (2.0 * Utility.randDouble() - 1.0));
			...
			double plankLength = minLength + Utility.randDouble() * (maxLength - minLength);
						
			Point3D p0=new Point3D(p0x ,y0, iz * plankWidth);
			Point3D p1=new Point3D(p0x + plankLength, y0 + plankThickness, (iz + 1) * plankWidth);
...
//	rectanglePtr.setMaterial(svMattePtr4);  // textured with Blue Glass
	...
	TInstance woodPtr1 = new TInstance(new Wood(new RGBColor(0.55, 0.43, 0.13), Utility.BLACK));
	...
	TInstance woodPtr2 = new TInstance(new Wood(new RGBColor(0.55, 0.43, 0.13), Utility.BLACK));
	woodPtr2.scale(2.0);
	...
	Triangle trianglePtr1 = new Triangle (new Point3D(-a, 0, b), new Point3D(-a - w, 0, b + w), new Point3D(a + w, 0, b + w));
	trianglePtr1.setMaterial(svMattePtr5);
	framedPicturePtr.addObject(trianglePtr1);
	
	Triangle trianglePtr2 = new Triangle (new Point3D(-a, 0, b), new Point3D(a + w, 0, b + w), new Point3D(a, 0, b));
	...
	Triangle trianglePtr3 = new Triangle (new Point3D(a, 0, -b), new Point3D(a + w, 0, -b - w), new Point3D(-a - w, 0, -b - w));
	trianglePtr3.setMaterial(svMattePtr5);
	framedPicturePtr.addObject(trianglePtr3);
	
	Triangle trianglePtr4 = new Triangle (new Point3D(a, 0, -b), new Point3D(-a - w, 0, -b - w), new Point3D(-a, 0, -b));
	...
	Triangle trianglePtr5 = new Triangle (new Point3D(a, 0, b), new Point3D(a + w, 0, b + w), new Point3D(a + w, 0, -b - w));
	trianglePtr5.setMaterial(svMattePtr6);
	framedPicturePtr.addObject(trianglePtr5);
	
	Triangle trianglePtr6 = new Triangle (new Point3D(a, 0, b), new Point3D(a + w, 0, -b - w), new Point3D(a, 0, -b));
	...
	Triangle trianglePtr7 = new Triangle (new Point3D(-a, 0, -b), new Point3D(-a - w, 0, -b - w), new Point3D(-a - w, 0, b + w));
	trianglePtr7.setMaterial(svMattePtr6);
	framedPicturePtr.addObject(trianglePtr7);
	
	Triangle trianglePtr8 = new Triangle (new Point3D(-a, 0, -b), new Point3D(-a - w, 0, b + w), new Point3D(-a, 0, b));
	...
	TInstance marblePtr = new TInstance(new RampFBmTexture(imagePtr4, numOctaves, fbmAmount));
...
//	gridPtr.setMaterial(svMattePtr7); 		// textured
	...
	for (int j = 0; j < numXboxes; j++) {
		...
		TInstance sandstonePtr = new TInstance(new RampFBmTexture(imagePtr5, numOctaves, fbmAmount));
		...
		sandstonePtr.translate(10.0 * (2.0 * Utility.randDouble() - 1.0), 20.0 * (2.0 * Utility.randDouble() - 1.0), 30.0 * (2.0 * Utility.randDouble() - 1.0));
		...
		BeveledBox boxPtr = new BeveledBox(	new Point3D(bathXmin + j * (xSize / numXboxes), 0, bathZmin), 
												new Point3D(bathXmin + (j + 1) * (xSize / numXboxes), bathHeight, bathZmin + thickness),
												bathBevelRadius);
	...
	for (int j = 0; j < numXboxes; j++) {
		...
		BeveledBox boxPtr = new BeveledBox(	new Point3D(bathXmin + j * (xSize / numXboxes), 0, bathZmax - thickness), 
												new Point3D(bathXmin + (j + 1) * (xSize / numXboxes), bathHeight, bathZmax),
												bathBevelRadius);
		boxPtr.setMaterial(svMattePtr);
		bathPtr.addObject(boxPtr);
	...
	for (int j = 0; j < numZboxes; j++) {
		...
		BeveledBox boxPtr = new BeveledBox(	new Point3D(bathXmin, 0, bathZmin + thickness + j * ((zSize - 2 * thickness) / numZboxes)),
												new Point3D(bathXmin + thickness, bathHeight, bathZmin + thickness + (j + 1) * ((zSize - 2 * thickness) / numZboxes)),
												bathBevelRadius);
		boxPtr.setMaterial(svMattePtr);
		bathPtr.addObject(boxPtr);
	...
	for (int j = 0; j < numZboxes; j++) {
		...
		BeveledBox boxPtr = new BeveledBox(	new Point3D(bathXmax - thickness, 0, bathZmin + thickness + j * ((zSize - 2 * thickness) / numZboxes)),
												new Point3D(bathXmax, bathHeight, bathZmin + thickness + (j + 1) * ((zSize - 2 * thickness) / numZboxes)),
												bathBevelRadius);
		boxPtr.setMaterial(svMattePtr);
		bathPtr.addObject(boxPtr);
	}
	...
	Rectangle waterSurfacePtr = new Rectangle(	new Point3D(bathXmin + thickness - bathBevelRadius, waterHeight, bathZmin + thickness - bathBevelRadius), 
...
*/
	...
	//		texturePtr.setMaxValue(1.0);   // original - now renders darker
			...
			Point3D p0=new Point3D(tilesXmin + ix * (tileSize + groutWidth), tilesYmin + iy * (tileSize + groutWidth), tilesZmin);
			Point3D p1=new Point3D(tilesXmin + (ix + 1) * tileSize + ix * groutWidth, tilesYmin + (iy + 1) * tileSize + iy * groutWidth, tilesZmax);
...
//			tilePtr.setMaterial(reflectorPtr);		// textured
	...
	Rectangle groutPtr = new Rectangle(	new Point3D(tilesXmin, bathHeight, tileThickness / 2.0 - tileBevelRadius * 0.666),
	...
	instancePtr.translate(0, 0, -0.99 * (tileThickness / 2.0 - tileBevelRadius * 0.666));
