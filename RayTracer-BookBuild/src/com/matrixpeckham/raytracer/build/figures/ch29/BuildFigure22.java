...
//            imagePtr.loadPPMFile(Thread.currentThread().getContextClassLoader().getResourceAsStream(imagePath+"EarthHighRes.ppm"));
	    imagePtr.loadPPMFile(Thread.currentThread().getContextClassLoader().
		    getResourceAsStream(imagePath + "BlueGlass.ppm"));
	...
	try {
	    //        gridPtr.readFlatUVTriangles(Thread.currentThread().getContextClassLoader().getResourceAsStream(meshPath+fileName));		// for Figure 29.22(a)
	    gridPtr.readSmoothUvTriangles(Thread.currentThread().
		    getContextClassLoader().getResourceAsStream(meshPath
			    + fileName));		// for Figure 29.22(b)
	} catch (IOException e) {
