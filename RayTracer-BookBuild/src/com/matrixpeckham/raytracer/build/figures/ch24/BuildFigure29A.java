...
// panoramic is declared in LightProbe.h, which is #included in World.h
	...
	double ymin = 0.0;
	double ymax = 100;
	double innerRadius = 75.0;
	double outerRadius = 150.0;
	
	ThickRing ringPtr = new ThickRing(ymin, ymax, innerRadius, outerRadius);
	ringPtr.setMaterial(reflectivePtr);
	
	Instance rotatedRingPtr = new Instance (ringPtr);
	rotatedRingPtr.rotateX(65);
	rotatedRingPtr.rotateY(-10);
	w.addObject(rotatedRingPtr);
