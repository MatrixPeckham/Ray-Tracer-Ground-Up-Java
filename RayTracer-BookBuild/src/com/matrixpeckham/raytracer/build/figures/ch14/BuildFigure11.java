...
public class BuildFigure11 implements BuildWorldFunction {
    ...
    public void build(World w) {
        ...
        pinhole.setEye(75, 30, 100);
        pinhole.setLookat(-1, 3.7, 0);
        pinhole.setViewDistance(800);
        pinhole.computeUVW();
        w.setCamera(pinhole);

        PointLight light = new PointLight();
        light.setLocation(new Point3D(15, 15, 2.5));
        light.setExp(2);//for (a)
        light.scaleRadiance(600);//for (a)
        //light.scaleRadiance(2);//for (b)
        w.addLight(light);

        Directional light2 = new Directional();
        light2.setDirection(new Vector3D(15, 15, 2.5));
        light2.scaleRadiance(2);
        //w.addLight(light2);

        Emissive emissive = new Emissive();
        emissive.setCe(Utility.WHITE);
        emissive.scaleRadiance(1);

        Sphere sphere = new Sphere(new Point3D(15, 15, 2.5), 0.75);
